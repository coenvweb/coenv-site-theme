...
//TODO: Should probably rename now that we added the exporter
...
class CoEnvMemberApiImporter {
    ...
    function init () {
        ...
        add_action( 'admin_init', array( $this, 'member_export' ) );
    }

    function add_import_page () {
        add_submenu_page( 'edit.php?post_type=faculty', 'Faculty Import', 'Faculty Import', 'manage_options', 'coenv-member-api-tools-import', array( $this, 'display_submenu_page' ) );
        add_submenu_page( 'edit.php?post_type=faculty', 'Faculty Export', 'Faculty Export', 'manage_options', 'coenv-member-api-tools-export', array( $this, 'display_export_page' ) );
    }


    function display_export_page() {
        $nonce = wp_create_nonce('member_export');
        ?>
        <div class="wrap">
                <div id="icon-tools" class="icon32"><br /></div>
                <h2><?php echo __( 'Faculty Export' ) ?></h2>

                <p>Export faculty members to a CSV file.</p>
                <p><strong>Important: This export will not match the import tool's format requirements.</strong></p>
                <p>CSV file will be formatted as follows:</p>
                ...
                <table>
                    ...
                    <tr>
                        ...
                        <td bgcolor="#eee">bfranklin@uw.edu</td>
                        <td bgcolor="#eee">206-123-4567</td>
                        <td bgcolor="#eee">depts.uw.edu/bfranklin</td>
                        <td bgcolor="#eee">twitter.com/bfranklin</td>
                        <td bgcolor="#eee">facebook.com/bfranklin</td>
                    </tr>
                </table>

                <br />

                <form id="coenv-member-api-faculty-export" enctype="multipart/form-data" method="post" action="edit.php?post_type=faculty&page=coenv-member-api-tools-export&_wpnonce=<?php echo $nonce; ?>">
                    <input type="hidden" name="export_members" value="1" />
    ...
    function member_export() {
        if(wp_verify_nonce($_REQUEST['_wpnonce'], 'member_export') && current_user_can('manage_options') && $_POST['export_members']) {

            $fp = fopen('php://output', 'w');
            header('Content-type: application/csv');
            header('Content-Disposition: attachment; filename=faculty_export_'.date('m-d-Y', time()).'.csv');

            $columns = array('Lastname', 'Firstname', 'Academic Title', 'Email', 'Phone', 'Website', 'Twitter', 'Facebook');
            fputcsv($fp, $columns);

            $query = new WP_Query(array('post_type'=>'faculty', 'post_status'=>'publish', 'posts_per_page'=>-1));

            foreach($query->posts as $post) {
                $row = array();
                $row[] = get_field('last_name', $post->ID);
                $row[] = get_field('first_name', $post->ID);
                $row[] = get_field('academic_title', $post->ID);
                $row[] = get_field('email', $post->ID);
                $row[] = get_field('phone', $post->ID);
                $row[] = get_field('website', $post->ID);
                $row[] = get_field('twitter', $post->ID);
                $row[] = get_field('facebook', $post->ID);
                fputcsv($fp, $row);
            }
            exit;
        }
