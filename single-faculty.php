...
$unit = ! empty( $member['units'][0] ) ? $member['units'][0] : array();
$unit_color = ! empty( $unit['color'] ) ? $unit['color'] : '#303030';
$unit_texture = ! empty( $unit['image'] ) ? $unit['image'] : '';
$hero_style = '--faculty-accent: ' . $unit_color . ';';

if ( ! empty( $unit_texture ) ) {
	$hero_style .= ' --faculty-texture: url("' . esc_url( $unit_texture ) . '");';
}
...
<section class="Faculty-single" role="main">
	...
	<header class="Faculty-member-header" style="<?php echo esc_attr( $hero_style ); ?>">
		<div class="container">
			<div class="Faculty-member-header-inner">
				<div class="Faculty-member-image">
					<div class="Faculty-member-image-inner">
						<?php if ( ! empty( $member['images']['large'] ) ) : ?>
							<img src="<?php echo esc_url( $member['images']['medium']['url'] ) ?>" alt="Portrait of <?php echo esc_attr( $member['full_name'] ) ?>" />
						<?php endif ?>
					</div><!-- .Faculty-member-image-inner -->
				</div><!-- .Faculty-member-image -->

				<div class="Faculty-member-vcard">
					<div class="Faculty-member-vcard-inner">
						<div class="Faculty-member-vcard-content">
							<div class="Faculty-member-vcard-content-inner">
								<?php if ( ! empty( $member['units'] ) ) : ?>
									<h3 class="Faculty-member-unit"><a href="<?php echo esc_url( $unit['url'] ) ?>" title="See all faculty members in <?php echo esc_attr( $unit['name'] ) ?>"><?php echo esc_html( $unit['name'] ) ?></a></h3>
								<?php endif ?>

								<h1 class="Faculty-member-name"><?php echo esc_html( $member['full_name'] ) ?></h1>

								<?php if ( ! empty( $member['academic_title'] ) ) : ?>
									<h2 class="Faculty-member-academic-title"><?php echo esc_html( $member['academic_title'] ) ?></h2>
								<?php endif ?>
									...
									<div class="Faculty-member-themes">
										...
										<ul>
											<?php foreach ( $member['themes'] as $theme ) : ?>
												<li><a href="<?php echo esc_url( $theme['url'] ) ?>" title="See all faculty members in <?php echo esc_attr( $theme['name'] ) ?>"><?php echo esc_html( $theme['name'] ) ?></a></li>
											<?php endforeach ?>
										</ul>
			...
			</div><!-- .Faculty-member-header-inner -->
		</div><!-- .container -->
			...
			<div class="side-col">
				...
				<ul class="Faculty-member-contact-list">
					...
					<?php if ( ! empty( $member['website'] ) ) : ?>
						<li><a href="<?php echo $member['website'] ?>" target="_blank" rel="noopener"><i class="icon-contact-link-web"></i> Website</a></li>
					<?php endif ?>

					<?php if ( ! empty( $member['twitter'] ) ) : ?>
						<li><a href="<?php echo $member['twitter'] ?>" target="_blank" rel="noopener"><i class="icon-contact-link-twitter"></i> Twitter</a></li>
					<?php endif ?>
						
					<?php if ( ! empty( $member['bluesky'] ) ) : ?>
						<li class="contact-link bluesky"><a href="<?php echo $member['bluesky'] ?>" target="_blank" rel="noopener"><svg width="13" height="13" viewBox="0 0 34 34" style="display: inline-table; "><g transform="scale(0.055 0.055)">
					...
					<?php if ( ! empty( $member['instagram'] ) ) : ?>
						<li class="contact-link instagram"><a href="<?php echo $member['instagram'] ?>" target="_blank" rel="noopener"><svg width="13" height="13" viewBox="0 0 34 34" style="display: inline-table; ">
					...
					<?php if ( ! empty( $member['facebook'] ) ) : ?>
						<li><a href="<?php echo $member['facebook'] ?>" target="_blank" rel="noopener"><i class="icon-contact-link-facebook"></i> Facebook</a></li>
					<?php endif ?>
